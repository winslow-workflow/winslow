...
class McpSessionPort:
    ...
    def history(self):
        return tuple(
            CODEC.decode(BatchOutcome, row) for row in _aslist(self._call("history"))
        )
    ...
    def log_tail(self, batch_uuid, key, limit=200):
        return [
            str(line)
            for line in _aslist(
                self._call("log_tail", batch_uuid=batch_uuid, task_key=key, limit=limit)
            )
        ]

    def caches(self):
        return tuple(
            CODEC.decode(CacheInfo, card) for card in _aslist(self._call("caches"))
        )
...
class McpAppPort:
    ...
    def create_session(self, workflow, overrides=None, values=None):
        ...
        return CODEC.decode(SessionInfo, row)

    def restore_session(self, session_id):
        row = _refuses(self._bridge.call("restore_session", {"session_id": session_id}))
        return CODEC.decode(SessionInfo, row)
